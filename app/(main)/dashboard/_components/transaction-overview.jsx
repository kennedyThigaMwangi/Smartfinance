...
function buildAlerts(savingsRate, budgets, expTrend, topCat, income, expense) {
  const alerts = [];
  if (savingsRate < 10)
    alerts.push({ level: "critical", icon: "🚨", title: "Critical: Savings Rate Below 10%", body: `You're only saving ${savingsRate}% of income. At this rate, financial resilience is at serious risk. Cut non-essential spending immediately.`, kes: null });
  if (savingsRate >= 20)
    alerts.push({ level: "success", icon: "🏆", title: "Excellent Savings Rate", body: `${savingsRate}% savings rate puts you in the top tier of financial health. Consider investing surplus for compound growth.`, kes: null });
  const overBudget = budgets.filter(b => b.pct > 100);
  overBudget.slice(0, 2).forEach(b =>
    alerts.push({ level: "warning", icon: "⚠️", title: `Budget Exceeded: ${b.cat}`, body: `Spending is ${b.pct}% of your typical monthly average. Consider reducing ${b.cat} expenses by ${fKes(b.current - b.budget)}.`, kes: `Over by ${fKes(b.current - b.budget)}` }));
  if (expTrend > income * 0.05)
    alerts.push({ level: "warning", icon: "📈", title: "Expenses Rising", body: "Your expenses increased significantly compared to last month. Without correction, savings will erode within 3-4 months.", kes: null });
  if (income > 0 && expense / income > 0.9)
    alerts.push({ level: "critical", icon: "🔥", title: "High Expense Ratio", body: `Expenses are ${Math.round((expense/income)*100)}% of income. You need an immediate spending audit to avoid going into debt.`, kes: null });
  if (alerts.length === 0)
    alerts.push({ level: "info", icon: "✅", title: "Finances On Track", body: "No critical alerts this period. Keep maintaining your spending discipline and savings habits.", kes: null });
...
function genRecs(income, expense, savingsRate, cats, trend, daysLeft) {
  ...
  const recs     = [];
  if (savingsRate < 10)
    recs.push({ icon:"💡", p:"high",   badge:"Critical",    title:"Savings rate critically low",         body:`Only ${savingsRate}% saved. Cut one major category and automate ${fKes(income*.05)}/month to a savings account.`, kes:`Target: ${fKes(income*.20)}/month` });
    ...
    recs.push({ icon:"🏆", p:"low",    badge:"Excellent",    title:"Strong savings — consider investing",  body:`${savingsRate}% savings rate. Allocate 30% of surplus to a unit trust or SACCO for compound growth.`,            kes:`Investable: ${fKes(net)}` });
  if (topCat && topCat.pct > 30)
    recs.push({ icon:"⚠️", p:"high",   badge:"Overspend",    title:`${topCat.name} at ${topCat.pct}% of expenses`, body:`High concentration in one category. Cap it at 25% to free up cash flow.`,                           kes:`Potential saving: ${fKes(topCat.val*.15)}/month` });
  if (expTrend > income * 0.05)
    recs.push({ icon:"🔺", p:"medium", badge:"Rising Costs", title:"Expenses trending upward",            body:`Expenses rose by ${fKes(expTrend)} last month. If unchecked, savings erode in 3-4 months.`,                    kes:`Burn rate: ${fKes(burn)}/day` });
  ...
  recs.push({ icon:"📅", p:"info",     badge:"Month Plan",   title:`${daysLeft} days left — finish strong`,body:`Projected spend: ${fKes(burn*30)}. Daily allowance remaining: ${fKes(net/Math.max(daysLeft,1))}.`,          kes:null });
  if (savingsRate > 15)
    recs.push({ icon:"🚀", p:"info",   badge:"Invest",       title:"Put idle savings to work",            body:`With a healthy rate, allocate ${fKes(net*.3)}/month to a diversified portfolio for compound growth.`,          kes:`5-yr at 12% p.a: ${fKes(net*.3*12*5*1.12)}` });
...
function genPredictions(income, expense, trend, savingsRate, topCat) {
  ...
  return [
    ...
    { emoji:"⚡", horizon:"Risk Alert",      title:topCat?`${topCat.name} Risk`:"Spending Risk", body:topCat?`If ${topCat.name} grows 5%/month it costs an extra ${fKes(topCat.val*.05*6)} over 6 months.`:"Diversify categories to reduce concentration risk.", kes:topCat?fKes(topCat.val*.05*6):null, positive:false },
...
// ═══════════════════════════════════════════════════════════════════════════════
// §10 · MONTH-OVER-MONTH COMPARISON PANEL
// ═══════════════════════════════════════════════════════════════════════════════
...
function CalendarHeatmap({ heatmapData }) {
  ...
  return (
    <div>
      <div className="dov-cal-grid" style={{ gridTemplateColumns:"repeat(7,1fr)" }}>
