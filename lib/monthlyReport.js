...
export async function getMonthlyReport(userId, year, month) {
  ...
  const dailyTrend = Array.from({ length: daysInMonth }, (_, i) => {
    const day        = i + 1;
    const dayIncome  = sumAmount(
      incomeList.filter((t) => new Date(t.date).getDate() === day)
    );
    const dayExpense = sumAmount(
      expenseList.filter((t) => new Date(t.date).getDate() === day)
    );
