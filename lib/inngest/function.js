...
import { getFinancialReport } from "@/lib/reports";          // ← replaces getMonthlyReport
import { generateInsights } from "@/actions/generate-report"; // ← shared insights helper
import { MonthlyReportEmail, BudgetWarningEmail, BudgetExceededEmail } from "@/emails/template";
  ...
  async ({ step }) => {
    ...
    for (const budget of budgets) {
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        // 🚨 EXCEEDED (100%+) — alert every 6 hours
        ...
        // ⚠️ WARNING (80–99%) — once per month
